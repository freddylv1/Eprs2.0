...
export const EPRS_PHONICS_RULES: Record<string, PhonicsRule> = {
  R001: {
    ...
    phonemeMapping: [
      { letters: 'a', ipa: '/æ/', desc: '蝴蝶音短母音（如 cat, bag, hand）' },
      { letters: 'e', ipa: '/ɛ/', desc: '扁唇短母音（如 bed, desk, pen）' },
      { letters: 'i', ipa: '/ɪ/', desc: '鬆母音短母音（如 sit, fish, milk）' },
      { letters: 'o', ipa: '/ɑː/', desc: '美式開口長短母音（如 hot, dog, stop）' },
      { letters: 'u', ipa: '/ʌ/', desc: '央短母音（如 cup, bus, sun）' }
    ],
    examples: [
  ...
  R003: {
    ...
    phonemeMapping: [
      { letters: 'a_e', ipa: '/eɪ/', desc: '如 cake, lake, name, safe' },
      { letters: 'e_e', ipa: '/iː/', desc: '如 Pete, scene, eve' },
      { letters: 'i_e', ipa: '/aɪ/', desc: '如 bike, time, kite, fine' },
      { letters: 'o_e', ipa: '/oʊ/', desc: '如 home, rope, nose, hope' },
      { letters: 'u_e', ipa: '/uː/ 或 /juː/', desc: '如 cute, tube, flute, June' }
    ],
    examples: [
...
/**
 * 輔助函數：從規則文字中提取出所有匹配的規則代碼 (如 R001, R003)
 */
export function extractRuleCodes(ruleString: string): string[] {
  if (!ruleString) return [];
  const matches = ruleString.match(/R\d{3}/gi);
  if (!matches) return [];
  return Array.from(new Set(matches.map(m => m.toUpperCase())));
}

/**
 * 輔助函數：根據代碼獲取規則資料，若無對應則返回通用預設
 */
